#!/usr/bin/env python3
"""MuseTalk lip-sync worker (task: ``video.lipsync``).

This is the HEAVY half of the add-on: it imports torch / diffusers /
transformers / cv2 and the ``musetalk`` package and runs the actual generative
pipeline. It never ships frozen — it runs inside the private cache virtual-env
that :mod:`musetalk_addon._bootstrap` provisions on first use, so the release
archive stays tiny. Lip-sync is done with **MuseTalk**
(https://github.com/TMElyralab/MuseTalk, MIT-licensed) — a generative,
audio-conditioned mouth-inpainting pipeline built on Stable Diffusion's VAE +
a UNet + Whisper audio features. It is GPU/CUDA only and needs several GB of
model weights, downloaded once on first use into ``~/.cache/subtitld/musetalk/``
(override with ``SUBTITLD_ADDON_CACHE``).

Wire protocol (one JSON object per line) — see the package docstring. When run
as a proxied worker (``--worker``) the bootstrap has already completed the
hello handshake with the host, so ``run_worker_loop`` is called with
``emit_hello=False``.
...
# ---------------------------------------------------------------------------
class _Inbox:
    """Frames read off stdin by a dedicated thread.

    A lip-sync job runs for minutes, and it runs on the main thread. If that
    thread were also the one reading stdin, a ``cancel`` sent mid-job would sit
    unread in the pipe until the job finished — making cancellation a no-op and
    the checkpoint between UNet batches dead code. Reading in the background
    instead lets ``cancel`` and ``shutdown`` be observed *while* the pipeline
    runs.
    """
        ...
        try:
            for line in sys.stdin:
                ...
                if ftype == 'ready':
                    ...
                    break
                elif ftype == 'video.lipsync':
                    self._requests.put(frame)
        except (ValueError, OSError):
            pass
        self._eof.set()
        self._requests.put(None)  # unblock a waiting next()

    def next_request(self):
        """Block until the next request arrives, or None when stdin is done."""
    ...
    def clear(self, req_id):
        with self._lock:
            self._cancelled.discard(req_id)


def run_worker_loop(emit_hello=True):
    """Drive the JSON-line protocol on stdin/stdout.

    When ``emit_hello`` is False the worker is running behind the bootstrap
    proxy, which has already completed the hello handshake with the host — so
    the worker must NOT emit its own hello (that would confuse the host).
    """
    if emit_hello:
        send({
            'type': 'hello',
    ...
    while True:
        frame = inbox.next_request()
        if frame is None:  # stdin closed / shutdown
            break

        req_id = frame.get('id')
        params = frame.get('params') or {}

        def progress(value, message='', _rid=req_id):
            send({'id': _rid, 'type': 'progress',
                  'value': max(0.0, min(1.0, round(float(value), 4))),
                  'message': message})

        def is_cancelled(_rid=req_id):
            return inbox.is_cancelled(_rid)
